locals {
  ...
  logic_app_no_identity_name      = "la-noid-${substr(local.unique_suffix, 0, 6)}"
  logic_app_queue_connection_name = "apic-queue-${substr(local.unique_suffix, 0, 6)}"
  ...
  monitor_dcr_name                = "dcr-linux-vm-${substr(local.unique_suffix, 0, 6)}"
  monitor_dcr_association_name    = "assoc-${substr(local.monitor_dcr_name, 0, 24)}"
  container_app_env_name          = "cae-ops-${substr(local.unique_suffix, 0, 6)}"
  container_app_name              = "ca-public-${substr(local.unique_suffix, 0, 6)}"
  container_app_job_manual_name   = "caj-manual-${substr(local.unique_suffix, 0, 6)}"
  container_app_job_schedule_name = "caj-sched-${substr(local.unique_suffix, 0, 6)}"
  container_app_job_event_name    = "caj-event-${substr(local.unique_suffix, 0, 6)}"
  app_insights_app_name           = "appi-app-${substr(local.unique_suffix, 0, 6)}"
  ...
  vm_extension_script_name        = "vm-extension-proof.sh"
  vm_web_extension_name           = "config-bootstrap"
  vmss_api_extension_name         = "maintenance-script"
  ...
  azure_ml_workspace_name         = trimspace(var.azure_ml_workspace_name) != "" ? trimspace(var.azure_ml_workspace_name) : "aml2-ops-${substr(local.unique_suffix, 0, 6)}"

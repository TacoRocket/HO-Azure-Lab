...
func TestTruthFirstBootstrapManifestTracksCoveredAndExceptedSurfaces(t *testing.T) {
	...
	if manifest.Surfaces.Commands["app-credentials"].ReasonCode != "first_gate_followup" {
		t.Fatalf("expected app-credentials follow-up, got %#v", manifest.Surfaces.Commands["app-credentials"])
	}
	if manifest.Surfaces.Families["compute-control"].ReasonCode != "first_gate_followup" {
		t.Fatalf("expected compute-control follow-up, got %#v", manifest.Surfaces.Families["compute-control"])
	}
}
